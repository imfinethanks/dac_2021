...
class LoadImagesAndLabels_file(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_labels=True, cache_images=False, single_cls=False,read_xml=True):
        # path = str(Path(path))  # os-agnostic
        ...
        for file_names in file_handle.readlines():
            self.img_files.append(file_names[:-1]+'.jpg')
            if read_xml:
                self.label_files.append(file_names[:-1]+'.xml')
            else:
                self.label_files.append('/home/d/data_training/whale1/720 (13)_1839.xml')
